...
#ifdef PROBABILISTIC
   std::cout << "occupancy_buffer_[idx] =" << occupancy_buffer_[idx] << " " << "min_occupancy_log_ =" << min_occupancy_log_ << std::endl;
...
// double fiesta::ESDFMap::Dist(const Eigen::Vector3i a, const Eigen::Vector3i b) {
//   Eigen::Vector3d delta = (a - b).cast<double>() * resolution_; // 关键：转换为实际距离
//   return delta.norm();
// }
...
void fiesta::ESDFMap::SetParameters(double p_hit, double p_miss, double p_min, double p_max, double p_occ) {
  std::cout << "p_hit" << p_hit << " " << p_miss << " " << p_min << " " << p_max << " " << " " << p_occ << std::endl;
  ...
  std::cout << "prob_hit_log_ =" << prob_hit_log_ << "prob_miss_log_ =" << prob_miss_log_ << "clamp_min_log_ =" << clamp_min_log_
  << " clamp_max_log_ =" << clamp_max_log_ << "min_occupancy_log_ = " << min_occupancy_log_ << std::endl;
}
#endif
...
//    UpdateOccupancy();
  ...
  while (!insert_queue_.empty()) {
    // std::cout << "ESDFMap::UpdateESDF() insert 1" << std::endl;
    ...
    if (Exist(idx)) {
      ...
      // std::cout << "ESDFMap::UpdateESDF() insert 2  xx.point_ =" <<  xx.point_ << std::endl;

      // 初始化邻域点的 closest_obstacle_
      #if 0
      for (const auto &dir : dirs_) {
        Eigen::Vector3i new_pos = Idx2Vox(idx) + dir;
        int new_pos_idx = Vox2Idx(new_pos);
        if (VoxInRange(new_pos)) {
          closest_obstacle_[new_pos_idx] = xx.point_;
          double tmp = Dist(Idx2Vox(new_pos_idx), xx.point_);
          if (distance_buffer_[new_pos_idx] > tmp) {
            distance_buffer_[new_pos_idx] = tmp;
  ...
  while (!delete_queue_.empty()) {
    // std::cout << "ESDFMap::UpdateESDF() delete 1" << std::endl;
  ...
  while (!update_queue_.empty()) {
    // std::cout << "ESDFMap::UpdateESDF() update 1" << std::endl;
...
//            QueueElement xx = update_queue_.top();
    ...
    {
      // std::cout << "ESDFMap::UpdateESDF() update 2" << std::endl;
      continue;
    }
    times++;
    bool change = false;
    for (int i = 0; i < num_dirs_; i++) {
      // std::cout << "ESDFMap::UpdateESDF() update 3" << std::endl;
      Eigen::Vector3i new_pos = xx.point_ + dirs_[i];
      if (VoxInRange(new_pos)) {
        int new_pos_idx = Vox2Idx(new_pos);
        if (closest_obstacle_[new_pos_idx](0) == undefined_)
        {
          // std::cout << "ESDFMap::UpdateESDF() update 4" << std::endl;
          continue;
        }
        //double tmp = Dist(xx.point_, closest_obstacle_[new_pos_idx]);
        double tmp = Dist(xx.point_, closest_obstacle_[new_pos_idx]);
        // std::cout << "ESDFMap::UpdateESDF() Dist =" <<tmp << std::endl;
        // std::cout << "xx.point_ =(" << xx.point_[0] << " " << xx.point_[1] << "," << xx.point_[2] << ")"<< std::endl;
        // std::cout << "new_pos =(" << new_pos[0] << "," << new_pos[1] << "," << new_pos[2] << ")" << std::endl;
...
int fiesta::ESDFMap::SetOccupancy(Eigen::Vector3d pos, int occ) {
  ...
  // std::cout << "SetOccupancy1" << std::endl;
  if (occ != 1 && occ != 0) {
...
int fiesta::ESDFMap::SetOccupancy(Eigen::Vector3i vox, int occ) {
  int idx = Vox2Idx(vox);//, idx2 = Vox2Idx(vox, 2);
    // std::cout << "SetOccupancy2" << std::endl;
  if (!VoxInRange(vox))
  {
     std::cout << "SetOccupancy3" << std::endl;
    return idx;
  }
  ...
  num_miss_[idx]++;
  num_hit_[idx] += occ;
    // std::cout << "SetOccupancy4 num_miss_ =" << num_miss_[idx]<< "id = "<< idx << "occupancy_buffer_[idx] =" << occupancy_buffer_[idx]<< std::endl;
  if (num_miss_[idx] == 1) {
#ifdef HASH_TABLE
    ...
    // multi-thread
    // mtx.lock();
    //  std::cout << "SetOccupancy5" << std::endl;
    occupancy_queue_.push(QueueElement{vox, 0.0});
    // mtx.unlock();
#endif
  }
...
double fiesta::ESDFMap::GetDistance(Eigen::Vector3i vox) {
  // std::cout << "distance_buffer_[Vox2Idx(vox)] =" << distance_buffer_[Vox2Idx(vox)] << std::endl;
